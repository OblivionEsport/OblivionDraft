...
async function setup() {
    let rawData = await fetch("/api/draft/full")
...
async function actions() {
    let rawData = await fetch("/api/draft/actions")
    if (rawData.status != 200) {
        is_connected = false;
        return;
    }
...
async function updateCounter() {
    let rawData = await fetch("/api/draft/timer")
    if (rawData.status != 200) {
        is_connected = false;
        return;
    }
...
async function getTeamInfo() {
    resp = await fetch("/api/admin/teams/full")
    if (resp.status != 200) {
        return;
    }
...
async function updateInfo() {
    for (let i = 0; i < players.length; i++) {
        let id = players[i].dataset.summonerId;
        let champID = players[i].dataset.championId;
        resp = await fetch(`/api/draft/summoner/${i}`)
        if (resp.status != 200) {
            is_connected = false;
            return;
        }
    ...
    resp = await fetch("/api/draft/bans/");
    data = await resp.json();
...
async function getPlayerInfo(id, champID) {
    resp = await fetch(`/api/draft/summoner/info/?summonerID=${id}&championID=${champID}`)
    data = await resp.json();
    return data;
}
